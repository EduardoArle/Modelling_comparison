...
#load selected projections
cat_mir_4.5_2050 <- list()
for(i in 1:length(cat_sel_TSS))
{
  cat_mir_4.5_2050[[i]] <- raster(paste0('Pred_P_bengalensis_',
                                         cat_sel_TSS[i], '.grd'))
  print(i)
}

#binarise projections according to threshold
cat_mir_4.5_2050_bin <- list()
for(i in 1:length(cat_mir_4.5_2050))
{
  cat_mir_4.5_2050_bin[[i]] <- cat_mir_4.5_2050[[i]]
  th <- eval_cat$threshold[cat_sel_TSS[i]]
  cat_mir_4.5_2050_bin[[i]][] <- ifelse(cat_mir_4.5_2050_bin[[i]][] >= th, 1, 0)
  print(i)
}

#stack all projections
cat_mir_4.5_2050_sum <- sum(stack(cat_mir_4.5_2050_bin))

plot(cat_mir_4.5_2050_sum, main = 'mir_4.5_2050')

#save raster layer
setwd(wd_ensemble)
writeRaster(cat_mir_4.5_2050_sum, 'Prionailurus_bengalensis_mir_4.5_2050.tif',
            format ="GTiff")

cat_mir_4.5_2050_sum <- raster('Prionailurus_bengalensis_mir_4.5_2050.tif')
...
#load selected projections
cat_mir_8.5_2050 <- list()
for(i in 1:length(cat_sel_TSS))
{
  cat_mir_8.5_2050[[i]] <- raster(paste0('Pred_P_bengalensis_',
                                         cat_sel_TSS[i], '.grd'))
  print(i)
}

#binarise projections according to threshold
cat_mir_8.5_2050_bin <- list()
for(i in 1:length(cat_mir_8.5_2050))
{
  cat_mir_8.5_2050_bin[[i]] <- cat_mir_8.5_2050[[i]]
  th <- eval_cat$threshold[cat_sel_TSS[i]]
  cat_mir_8.5_2050_bin[[i]][] <- ifelse(cat_mir_8.5_2050_bin[[i]][] >= th, 1, 0)
  print(i)
}

#stack all projections
cat_mir_8.5_2050_sum <- sum(stack(cat_mir_8.5_2050_bin))

plot(cat_mir_8.5_2050_sum, main = 'mir_8.5_2050')

#save raster layer
setwd(wd_ensemble)
writeRaster(cat_mir_8.5_2050_sum, 'Prionailurus_bengalensis_mir_8.5_2050.tif',
            format ="GTiff")

cat_mir_8.5_2050_sum <- raster('Prionailurus_bengalensis_mir_8.5_2050.tif')
...
#sum all layers and calculate percentage of agreement
cat_2050_ens <- sum(stack(cat_had_4.5_2050_sum, cat_had_8.5_2050_sum,
                          cat_mir_4.5_2050_sum, cat_mir_8.5_2050_sum)) /
               (length(cat_sel_TSS) * 4)  * 100

par(mfrow = c(1,2))
plot(cat_pr_ens, main = 'Present')
plot(cat_2050_ens, main = '2050')
par(mfrow = c(1,1))

#save raster layer
setwd(wd_ensemble)
writeRaster(cat_2050_ens, 'Prionailurus_bengalensis_2050.tif',
            format ="GTiff")

cat_2050_ens <- raster('Prionailurus_bengalensis_2050.tif')


#save only HadGEM model
...
#load selected projections
cat_mir_4.5_2090 <- list()
for(i in 1:length(cat_sel_TSS))
{
  cat_mir_4.5_2090[[i]] <- raster(paste0('Pred_P_bengalensis_',
                                         cat_sel_TSS[i], '.grd'))
  print(i)
}

#binarise projections according to threshold
cat_mir_4.5_2090_bin <- list()
for(i in 1:length(cat_mir_4.5_2090))
{
  cat_mir_4.5_2090_bin[[i]] <- cat_mir_4.5_2090[[i]]
  th <- eval_cat$threshold[cat_sel_TSS[i]]
  cat_mir_4.5_2090_bin[[i]][] <- ifelse(cat_mir_4.5_2090_bin[[i]][] >= th, 1, 0)
  print(i)
}

#stack all projections
cat_mir_4.5_2090_sum <- sum(stack(cat_mir_4.5_2090_bin))

plot(cat_mir_4.5_2090_sum, main = 'mir_4.5_2090')

#save raster layer
setwd(wd_ensemble)
writeRaster(cat_mir_4.5_2090_sum, 'Prionailurus_bengalensis_mir_4.5_2090.tif',
            format ="GTiff")
...
#load selected projections
cat_mir_8.5_2090 <- list()
for(i in 1:length(cat_sel_TSS))
{
  cat_mir_8.5_2090[[i]] <- raster(paste0('Pred_P_bengalensis_',
                                         cat_sel_TSS[i], '.grd'))
  print(i)
}

#binarise projections according to threshold
cat_mir_8.5_2090_bin <- list()
for(i in 1:length(cat_mir_8.5_2090))
{
  cat_mir_8.5_2090_bin[[i]] <- cat_mir_8.5_2090[[i]]
  th <- eval_cat$threshold[cat_sel_TSS[i]]
  cat_mir_8.5_2090_bin[[i]][] <- ifelse(cat_mir_8.5_2090_bin[[i]][] >= th, 1, 0)
  print(i)
}

#stack all projections
cat_mir_8.5_2090_sum <- sum(stack(cat_mir_8.5_2090_bin))

plot(cat_mir_8.5_2090_sum, main = 'mir_8.5_2090')

#save raster layer
setwd(wd_ensemble)
writeRaster(cat_mir_8.5_2090_sum, 'Prionailurus_bengalensis_mir_8.5_2090.tif',
            format ="GTiff")
...
#sum all layers and calculate percentage of agreement
cat_2090_ens <- sum(stack(cat_had_4.5_2090_sum, cat_had_8.5_2090_sum,
                          cat_mir_4.5_2090_sum, cat_mir_8.5_2090_sum)) /
  (length(cat_sel_TSS) * 4)  * 100

par(mfrow = c(1,3))
plot(cat_pr_ens, main = 'Present')
plot(cat_2050_ens, main = '2050')
plot(cat_2090_ens, main = '2090')
par(mfrow = c(1,1))

#save raster layer
setwd(wd_ensemble)
writeRaster(cat_pr_ens, 'Prionailurus_bengalensis_2090.tif',
            format ="GTiff")
...
#load evaluation metrics
setwd(wd_evaluation)
eval_plant <- read.csv('Eval_P_bengalensis.csv')
